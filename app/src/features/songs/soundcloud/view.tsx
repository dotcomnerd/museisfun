...
const getSongs = async () => {
  const res = await api.get<Song[]>("/api/songs/soundcloud");
  return res.data;
};
...
export function SoundCloudSongsView() {
  ...
  const query = useQuery({ queryKey: ["songs", "soundcloud"], queryFn: getSongs });
  ...
  const removeSong = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["songs", "soundcloud"] });
    },
  });
